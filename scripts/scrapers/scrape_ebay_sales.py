...
Usage:
    # Scrape by card name
    python scripts/scrapers/scrape_ebay_sales.py --card-name "Charizard Base Set"

    # Scrape by set name (scrapes top cards in that set)
    python scripts/scrapers/scrape_ebay_sales.py --set-name "Base Set"

    # Control pages and rate limiting
    python scripts/scrapers/scrape_ebay_sales.py --card-name "Pikachu" --max-pages 5

    # Dry run (scrape + parse but don't insert)
    python scripts/scrapers/scrape_ebay_sales.py --card-name "Charizard Base Set" --dry-run

    # Scrape all WotC era sets
    python scripts/scrapers/scrape_ebay_sales.py --wotc

    # Scrape current era sets
    python scripts/scrapers/scrape_ebay_sales.py --current

    # Resume interrupted batch (skips already-scraped queries via log)
    python scripts/scrapers/scrape_ebay_sales.py --set-name "Base Set" --resume
"""
